...
import { getLastOrg, getLastProject } from "@/lib/navigation-context";
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Smart redirect: check last context first
      const lastOrg = getLastOrg();
      const lastProject = getLastProject();

      if (lastOrg && lastProject) {
        router.push(`/org/${lastOrg}/projects/${lastProject}/board`);
      } else if (lastOrg) {
        router.push(`/org/${lastOrg}`);
      } else {
        // No saved context — fetch orgs
        try {
          const orgsRes = await api.get<any>("/api/v1/organizations");
    ...
    <>
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-surface-400">
          Sign in to manage your projects and deployments
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Submit */}
        <Button type="submit" loading={loading} className="w-full !py-2.5 !text-sm font-semibold bg-accent-500 hover:bg-accent-600 shadow-lg shadow-accent-500/25 hover:shadow-xl hover:shadow-accent-500/30 hover:-translate-y-0.5 transition-all">
          Sign In
        </Button>
      </form>
